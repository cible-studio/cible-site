...
<head>
    ...
    {{-- Google Analytics 4 — injecté SEULEMENT si l'ID de mesure est
         défini dans .env (CIBLE_GA_ID). Sur dev/local, la variable est
         vide → aucun script chargé → aucune pollution des stats. --}}
    @if($gaId = config('services.cible.ga_id'))
    ...
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    /* CTA nav 2026-08-10 : le libellé est passé de "Demander un devis" (17
       car.) à "Recevoir ma recommandation média" (32 car.). On resserre la
       pastille et on la masque plus tôt (1200px au lieu de 1000px) pour
       qu'elle ne compresse pas les 5 liens de navigation. */
    header.site .bouton{font-size:13px;padding:12px 20px;white-space:nowrap;flex-shrink:0}
    @media(max-width:1200px){header.site .bouton{display:none}}
...
<header class="site">
    <nav class="nav">
        ...
        <a href="{{ route('home') }}" class="logo-w" aria-label="CIBLE, accueil">
        ...
        <div class="liens" id="site-links">
            ...
            <a href="{{ route('reseau') }}" {{ $current==='reseau' ? 'aria-current=page' : '' }}>Notre réseau</a>
            <a href="{{ route('contact') }}" {{ $current==='contact' ? 'aria-current=page' : '' }}>Contact</a>
        </div>
        {{-- CTA unifié 2026-08-10 : "Demander un devis" retiré partout au
             profit de "Recevoir ma recommandation média" (doc textes). --}}
        <a class="bouton b-rouge" href="{{ route('contact') }}">Recevoir ma recommandation média</a>
